...
#ifndef MATH_CUBIC_SPLINES_H_
#define MATH_CUBIC_SPLINES_H_
...
class CubicSpline {
 ...
 public:
  inline void setConditions(const Eigen::Vector2d& headPos, const Eigen::Vector2d& tailPos, const int& pieceNum) {
    N = pieceNum;
    headP = headPos;
    tailP = tailPos;
    A.create(4 * N, 4, 4);
  ...
  inline void getCurve(CubicCurve& curve) const {
    ...
    curve.reserve(N);
    for (int i = 0; i < N; ++i) {
      curve.emplace_back(1.0, b.block<4, 2>(4 * i, 0).transpose().rowwise().reverse());
    }
    return;
  }
  ...
  inline void getStretchEnergy(double& energy) const {
    energy = 0.0;
    for (int i = 0; i < N; ++i) {
      energy += 4.0 * b.row(4 * i + 2).squaredNorm() + 12.0 * b.row(4 * i + 2).dot(b.row(4 * i + 3)) +
  ...
  inline void getGrad(Eigen::Ref<Eigen::Matrix2Xd> gradByPoints) const {
    ...
    for (int i = 0; i < N; ++i) {
      gdC.row(4 * i + 3) = 12.0 * b.row(4 * i + 2) + 24.0 * b.row(4 * i + 3);
      gdC.row(4 * i + 2) = 8.0 * b.row(4 * i + 2) + 12.0 * b.row(4 * i + 3);
