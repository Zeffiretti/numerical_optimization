...
__global__ EIGEN_HIP_LAUNCH_BOUNDS_1024 void ReductionInitFullReduxKernelHalfFloat(
    Reducer reducer, const Self input, Index num_coeffs, packet_traits<Eigen::half>::type* scratch) {
      ...
      if (rem != 0) {
        ...
        *scratch = reducer.template initializePacket<PacketType>();
        for (int i = 0; i < rem / 2; i++) {
          *p_scratch = __halves2half2(input.m_impl.coeff(num_coeffs - packet_width + 2 * i),
...
struct FullReductionLauncher<
    ...
    typename internal::enable_if<
        internal::is_same<float, OutputType>::value || internal::is_same<double, OutputType>::value, void>::type> {
...
struct InnerReductionLauncher<
  ...
  static bool run(const Self& self, Op& reducer, const GpuDevice& device, OutputType* output,
    ...
    if (num_blocks > 1) {
      ...
      const int num_blocks = numext::mini<int>(max_blocks, dyn_blocks);
      LAUNCH_GPU_KERNEL((ReductionInitKernel<OutputType, Index>), num_blocks, 1024, 0, device, reducer.initialize(),
...
#  endif
  ...
  static bool run(const Self& self, Op& reducer, const GpuDevice& device, float* output,
    ...
    if (num_blocks > 1) {
      ...
      const int max_blocks = device.getNumGpuMultiProcessors() * device.maxGpuThreadsPerMultiProcessor() / 1024;
      const int num_blocks = numext::mini<int>(max_blocks, dyn_blocks);
      LAUNCH_GPU_KERNEL((ReductionInitKernel<float, Index>), num_blocks, 1024, 0, device, reducer.initialize(),
                        num_preserved_vals, output);
    }
