...
struct generic_i0 {
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE T run(const T& x) { return pmul(pexp(pabs(x)), generic_i0e<T, ScalarType>::run(x)); }
};
...
struct generic_i1 {
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE T run(const T& x) { return pmul(pexp(pabs(x)), generic_i1e<T, ScalarType>::run(x)); }
};
