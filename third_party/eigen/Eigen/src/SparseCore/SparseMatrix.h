 ...
 public:
  ...
  void conservativeResize(Index rows, Index cols) {
    ...
    if (outerChange > 0) {
      ...
      for (Index i = m_outerSize; i < m_outerSize + outerChange + 1; i++) m_outerIndex[i] = lastIdx;
    }
...
#  include EIGEN_SPARSEMATRIX_PLUGIN
 ...
 protected:
   ...
   * expression. 2 - otherwise, for each diagonal coeff, 2.a - if it already exists, then we update it, 2.b - otherwise,
   * if *this is uncompressed and that the current inner-vector has empty room for at least 1 element, then we perform
   * an in-place insertion. 2.c - otherwise, we'll have to reallocate and copy everything, so instead of doing so for
   * each new element, it is recorded in a std::vector. 3 - at the end, if some entries failed to be inserted in-place,
   * then we alloc a new buffer, copy each chunk at the right position, and insert the new elements.
   *
  ...
  void assignDiagonal(const DiagXpr diagXpr, const Func& assignFunc) {
    ...
    if (m_data.size() == 0 || overwrite) {
      ...
      if (n_entries > 0) {
        ...
        {
          internal::smart_copy(m_data.valuePtr() + prev_p, m_data.valuePtr() + m_data.size(),
                               newData.valuePtr() + prev_p + n_entries);
          internal::smart_copy(m_data.indexPtr() + prev_p, m_data.indexPtr() + m_data.size(),
                               newData.indexPtr() + prev_p + n_entries);
          for (Index j = prev_i + 1; j <= m_outerSize; ++j) m_outerIndex[j] += n_entries;
        }
        m_data.swap(newData);
      }
    }
...
template <typename _Scalar, int _Options, typename _StorageIndex>
typename SparseMatrix<_Scalar, _Options, _StorageIndex>::Scalar& SparseMatrix<_Scalar, _Options, _StorageIndex>::insert(
    Index row, Index col) {
  eigen_assert(row >= 0 && row < rows() && col >= 0 && col < cols());
