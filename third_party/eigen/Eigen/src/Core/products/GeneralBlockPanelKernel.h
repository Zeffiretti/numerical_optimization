...
template <typename _LhsScalar, typename _RhsScalar, bool _ConjLhs = false, bool _ConjRhs = false,
          int Arch = Architecture::Target, int _PacketSize = GEBPPacketFull>
class gebp_traits;
...
#define PACKET_DECL_COND_SCALAR_PREFIX(prefix, packet_size)                                    \
  typedef typename packet_conditional<                                                         \
      ...
      typename unpacket_traits<typename packet_traits<Scalar>::half>::half>::type prefix##ScalarPacket
...
DoublePacket<typename unpacket_traits<Packet>::half> predux_half_dowto4(
    const DoublePacket<Packet>& a, typename enable_if<unpacket_traits<Packet>::size == 16>::type* = 0) {
...
struct gebp_kernel {
  ...
  EIGEN_DONT_INLINE
  void operator()(const DataMapper& res, const LhsScalar* blockA, const RhsScalar* blockB, Index rows, Index depth,
                  Index cols, ResScalar alpha, Index strideA = -1, Index strideB = -1, Index offsetA = 0,
                  Index offsetB = 0);
};
      ...
      for (Index i = peeled_mc_quarter; i < rows; i += 1) {
        ...
        if ((SwappedTraits::LhsProgress % 4) == 0 && (SwappedTraits::LhsProgress <= 16) &&
          ...
          if (SwappedTraits::LhsProgress == 8) {
            ...
            typedef typename conditional<SwappedTraits::LhsProgress >= 8, typename unpacket_traits<SResPacket>::half,
                                         SResPacket>::type SResPacketHalf;
            typedef typename conditional<SwappedTraits::LhsProgress >= 8, typename unpacket_traits<SLhsPacket>::half,
                                         SLhsPacket>::type SLhsPacketHalf;
            typedef typename conditional<SwappedTraits::LhsProgress >= 8, typename unpacket_traits<SRhsPacket>::half,
                                         SRhsPacket>::type SRhsPacketHalf;
            typedef typename conditional<SwappedTraits::LhsProgress >= 8, typename unpacket_traits<SAccPacket>::half,
