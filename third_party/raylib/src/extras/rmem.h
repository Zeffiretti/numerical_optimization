...
//----------------------------------------------------------------------------------
static inline size_t __AlignSize(const size_t size, const size_t align) { return (size + (align - 1)) & -align; }
...
static MemNode* __FindMemNode(AllocList* const list, const size_t bytes) {
  for (MemNode* node = list->head; node != NULL; node = node->next) {
    ...
    // close in size - reduce fragmentation by not splitting.
      ...
      return __SplitMemNode(node, bytes);
  }
  return NULL;
}
...
MemPool CreateMemPoolFromBuffer(void* const restrict buf, const size_t size) {
  MemPool mempool = {0};
  if ((size == 0) || (buf == NULL) || (size <= sizeof(MemNode)))
    return mempool;
  else {
    mempool.arena.size = size;
    mempool.arena.mem = (uintptr_t)buf;
...
void* MemPoolAlloc(MemPool* const mempool, const size_t size) {
  if ((size == 0) || (size > mempool->arena.size))
    return NULL;
  else {
    ...
    if (new_mem == NULL) {
      ...
      if ((mempool->arena.offs - ALLOC_SIZE) < mempool->arena.mem)
        return NULL;
      else {
...
void* MemPoolRealloc(MemPool* const restrict mempool, void* const ptr, const size_t size) {
  ...
  // NULL ptr should make this work like regular Allocation.
  ...
  else if ((uintptr_t)ptr - sizeof(MemNode) < mempool->arena.mem)
    return NULL;
  else {
    ...
    if (resized_block == NULL)
      ...
      memmove(resized_block, ptr,
              (node->size > resized->size) ? (resized->size - NODE_SIZE) : (node->size - NODE_SIZE));
...
void MemPoolFree(MemPool* const restrict mempool, void* const ptr) {
  ...
  if ((ptr == NULL) || (p - sizeof(MemNode) < mempool->arena.mem))
    return;
  else {
    ...
    if ((block < mempool->arena.offs) || ((block - mempool->arena.mem) > mempool->arena.size) ||
        (mem_node->size == 0) || (mem_node->size > mempool->arena.size))
      return;
    // If the mem_node is right at the arena offs, then merge it back to the arena.
      ...
      struct AllocList* const l =
          (BUCKET_SLOT < MEMPOOL_BUCKET_SIZE) ? &mempool->buckets[BUCKET_SLOT] : &mempool->large;
...
size_t GetMemPoolFreeMemory(const MemPool mempool) {
  ...
  for (size_t i = 0; i < MEMPOOL_BUCKET_SIZE; i++)
    for (MemNode* n = mempool.buckets[i].head; n != NULL; n = n->next) total_remaining += n->size;
...
ObjPool CreateObjPoolFromBuffer(void* const restrict buf, const size_t objsize, const size_t len) {
  ...
  const size_t aligned_size = __AlignSize(objsize, sizeof(size_t));
  if ((buf == NULL) || (len == 0) || (objsize < sizeof(size_t)) || (objsize * len != aligned_size * len))
    return objpool;
  else {
    objpool.objSize = aligned_size;
    objpool.memSize = objpool.freeBlocks = len;
...
void ObjPoolFree(ObjPool* const restrict objpool, void* const ptr) {
  ...
  if ((ptr == NULL) || (block < objpool->mem) || (block > objpool->mem + objpool->memSize * objpool->objSize))
    return;
  else {
    // When we free our pointer, we recycle the pointer space to store the previous index and then we push it as our new
    // head. *p = index of Head in relation to the buffer; Head = p;
...
void* BiStackAllocFront(BiStack* const restrict destack, const size_t len) {
  if (destack->mem == 0)
    ...
    if (destack->front + ALIGNED_LEN >= destack->back)
      return NULL;
    else {
...
void* BiStackAllocBack(BiStack* const restrict destack, const size_t len) {
  if (destack->mem == 0)
    ...
    if (destack->back - ALIGNED_LEN <= destack->front)
      return NULL;
    else {
...
inline intptr_t BiStackMargins(const BiStack destack) { return destack.back - destack.front; }
