...
PHYSACDEF PhysicsBody CreatePhysicsBodyCircle(
    Vector2 pos, float radius, float density);  // Creates a new circle physics body with generic parameters
PHYSACDEF PhysicsBody
CreatePhysicsBodyRectangle(Vector2 pos, float width, float height,
                           float density);  // Creates a new rectangle physics body with generic parameters
PHYSACDEF PhysicsBody CreatePhysicsBodyPolygon(
    Vector2 pos, float radius, int sides, float density);  // Creates a new polygon physics body with generic parameters
...
PHYSACDEF void PhysicsShatter(
    PhysicsBody body, Vector2 position,
    float force);  // Shatters a polygon shape physics body to little physics bodies with explosion force
...
PHYSACDEF Vector2 GetPhysicsShapeVertex(
    PhysicsBody body,
    int vertex);  // Returns transformed position of a body shape (body position + vertex transformed position)
#  if defined(__cplusplus)
}
#  endif
...
static PhysicsVertexData CreateDefaultPolygon(
    float radius, int sides);  // Creates a random polygon shape with max vertex distance from polygon pivot
static PhysicsVertexData CreateRectanglePolygon(
    Vector2 pos, Vector2 size);  // Creates a rectangle polygon shape based on a min and max positions

static void InitializePhysicsManifolds(PhysicsManifold manifold);  // Initializes physics manifolds to solve collisions
static PhysicsManifold CreatePhysicsManifold(PhysicsBody a,
                                             PhysicsBody b);   // Creates a new physics manifold to solve collision
...
static void SolveCircleToPolygon(
    PhysicsManifold manifold);  // Solves collision between a circle to a polygon shape physics bodies
static void SolvePolygonToCircle(
    PhysicsManifold manifold);  // Solves collision between a polygon to a circle shape physics bodies
...
static void CorrectPhysicsPositions(
    PhysicsManifold manifold);  // Corrects physics bodies positions based on manifolds collision information
...
static inline float MathVector2SqrDistance(Vector2 v1,
                                           Vector2 v2);      // Returns the square root of distance between two vectors
...
static inline Vector2 MathMatVector2Product(Matrix2x2 matrix,
                                            Vector2 vector);  // Returns product between matrix 2x2 and vector
static int MathVector2Clip(Vector2 normal, Vector2* faceA, Vector2* faceB,
                           float clip);  // Returns clipping value based on a normal and two faces
static Vector2 MathTriangleBarycenter(Vector2 v1, Vector2 v2,
                                      Vector2 v3);  // Returns the barycenter of a triangle given by 3 points
  ...
  if (body != NULL) {
    if (body->shape.type == PHYSICS_POLYGON) {
      ...
      for (unsigned int i = 0; i < vertexData.vertexCount; i++) {
        ...
        Vector2 positionB =
            MathMatVector2Product(body->shape.transform, MathVector2Add(body->position, vertexData.positions[i]));
        unsigned int nextIndex = (((i + 1) < vertexData.vertexCount) ? (i + 1) : 0);
        Vector2 positionC = MathMatVector2Product(body->shape.transform,
                                                  MathVector2Add(body->position, vertexData.positions[nextIndex]));
      ...
      if (collision) {
        ...
        for (int i = 0; i < count; i++) {
          ...
          PhysicsBody body =
              CreatePhysicsBodyPolygon(center, 10, 3, 10);  // Create polygon physics body with relevant values
          ...
          pointB.x /= 2.0f;
          pointB.y /= 2.0f;
          Vector2 forceDirection = MathVector2Subtract(
              MathVector2Add(pointA, MathVector2Add(vertexData.positions[0], pointB)), body->position);
...
int GetPhysicsBodiesCount(void) { return physicsBodiesCount; }
    ...
    switch (body->shape.type) {
      case PHYSICS_CIRCLE: {
        position.x = body->position.x +
                     cosf(360.0f / PHYSAC_DEFAULT_CIRCLE_VERTICES * vertex * PHYSAC_DEG2RAD) * body->shape.radius;
        position.y = body->position.y +
                     sinf(360.0f / PHYSAC_DEFAULT_CIRCLE_VERTICES * vertex * PHYSAC_DEG2RAD) * body->shape.radius;
      } break;
      case PHYSICS_POLYGON: {
        PhysicsVertexData vertexData = body->shape.vertexData;
        position =
            MathVector2Add(body->position, MathMatVector2Product(body->shape.transform, vertexData.positions[vertex]));
      } break;
      default:
        break;
    }
...
void ClosePhysics(void) {
  ...
  if ((physicsBodiesCount > 0) || (usedMemory != 0)) {
    ...
    TRACELOG("[PHYSAC] Physics module closed successfully\n");
}
...
void SetPhysicsTimeStep(double delta) { deltaTime = delta; }
...
static double GetCurrentTime(void) { return (double)(GetClockTicks() - baseClockTicks) / frequency * 1000; }
      ...
      for (unsigned int j = i + 1; j < physicsBodiesCount; j++) {
        ...
        if (bodyB != NULL) {
          ...
          if (manifold->contactsCount > 0) {
            // Create a new manifold with same information as previously solved manifold and add it to the manifolds
            // pool last slot
            PhysicsManifold manifold = CreatePhysicsManifold(bodyA, bodyB);
...
static void SolvePhysicsManifold(PhysicsManifold manifold) {
  ...
  // Update physics body grounded state if normal direction is down and grounded state is not set yet in previous
  // manifolds
...
static void SolveCircleToCircle(PhysicsManifold manifold) {
  ...
  if (distance == 0.0f) {
    ...
    manifold->normal = CLITERAL(Vector2){
        normal.x / distance,
        normal.y / distance};  // Faster than using MathVector2Normalize() due to sqrt is already performed
    manifold->contacts[0] = CLITERAL(Vector2){manifold->normal.x * bodyA->shape.radius + bodyA->position.x,
...
static void SolveCircleToPolygon(PhysicsManifold manifold) {
  ...
  center =
      MathMatVector2Product(MathMatTranspose(bodyB->shape.transform), MathVector2Subtract(center, bodyB->position));
  ...
  for (unsigned int i = 0; i < vertexData.vertexCount; i++) {
    float currentSeparation =
        MathVector2DotProduct(vertexData.normals[i], MathVector2Subtract(center, vertexData.positions[i]));
...
static void SolvePolygonToPolygon(PhysicsManifold manifold) {
  ...
  // MathVector2Clip incident face to reference face side planes (due to floating point error, possible to not have
  // required points
  if (MathVector2Clip(CLITERAL(Vector2){-sidePlaneNormal.x, -sidePlaneNormal.y}, &incidentFace[0], &incidentFace[1],
                      negSide) < 2)
    return;
...
static void InitializePhysicsManifolds(PhysicsManifold manifold) {
  ...
  for (unsigned int i = 0; i < manifold->contactsCount; i++) {
    ...
    // The idea is if the only thing moving this object is gravity, then the collision should be performed without any
    // restitution
    if (MathVector2SqrLen(radiusV) < (MathVector2SqrLen(CLITERAL(Vector2){(float)(gravityForce.x * deltaTime / 1000),
                                                                          (float)(gravityForce.y * deltaTime / 1000)}) +
                                      PHYSAC_EPSILON))
...
static void IntegratePhysicsImpulses(PhysicsManifold manifold) {
  ...
  for (unsigned int i = 0; i < manifold->contactsCount; i++) {
    ...
    Vector2 radiusV = {0.0f, 0.0f};
    radiusV.x = bodyB->velocity.x + MathVector2Product(radiusB, bodyB->angularVelocity).x - bodyA->velocity.x -
                MathVector2Product(radiusA, bodyA->angularVelocity).x;
    radiusV.y = bodyB->velocity.y + MathVector2Product(radiusB, bodyB->angularVelocity).y - bodyA->velocity.y -
                MathVector2Product(radiusA, bodyA->angularVelocity).y;
    ...
    if (bodyA->enabled) {
      ...
      if (!bodyA->freezeOrient)
        bodyA->angularVelocity +=
            bodyA->inverseInertia * MathVector2CrossProduct(radiusA, CLITERAL(Vector2){-impulseV.x, -impulseV.y});
    }
    ...
    Vector2 tangent = {radiusV.x - (manifold->normal.x * MathVector2DotProduct(radiusV, manifold->normal)),
                       radiusV.y - (manifold->normal.y * MathVector2DotProduct(radiusV, manifold->normal))};
    ...
    if (absImpulseTangent < impulse * manifold->staticFriction)
      tangentImpulse = CLITERAL(Vector2){tangent.x * impulseTangent, tangent.y * impulseTangent};
    else
      tangentImpulse = CLITERAL(Vector2){tangent.x * -impulse * manifold->dynamicFriction,
    ...
    if (bodyA->enabled) {
      ...
      if (!bodyA->freezeOrient)
        bodyA->angularVelocity +=
            ...
            MathVector2CrossProduct(radiusA, CLITERAL(Vector2){-tangentImpulse.x, -tangentImpulse.y});
    }
...
static void CorrectPhysicsPositions(PhysicsManifold manifold) {
  ...
  correction.x = (PHYSAC_MAX(manifold->penetration - PHYSAC_PENETRATION_ALLOWANCE, 0.0f) /
                  (bodyA->inverseMass + bodyB->inverseMass)) *
                 manifold->normal.x * PHYSAC_PENETRATION_CORRECTION;
  correction.y = (PHYSAC_MAX(manifold->penetration - PHYSAC_PENETRATION_ALLOWANCE, 0.0f) /
                  (bodyA->inverseMass + bodyB->inverseMass)) *
...
static void FindIncidentFace(Vector2* v0, Vector2* v1, PhysicsShape ref, PhysicsShape inc, int index) {
  ...
  referenceNormal =
      MathMatVector2Product(MathMatTranspose(inc.transform), referenceNormal);  // To incident's model space
...
static inline float MathVector2CrossProduct(Vector2 v1, Vector2 v2) { return (v1.x * v2.y - v1.y * v2.x); }

// Returns the len square root of a vector
static inline float MathVector2SqrLen(Vector2 vector) { return (vector.x * vector.x + vector.y * vector.y); }

// Returns the dot product of two vectors
static inline float MathVector2DotProduct(Vector2 v1, Vector2 v2) { return (v1.x * v2.x + v1.y * v2.y); }
