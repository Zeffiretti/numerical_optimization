...
// -------------------------------------------
// void jar_mod_fillbuffer( jar_mod_context_t * modctx, short * outbuffer, unsigned long nbsample,
...
//   The output format is by default signed 48000Hz 16-bit Stereo PCM samples, otherwise it is changed with
//   jar_mod_setcfg(). The output buffer size in bytes must be equal to ( nbsample * 2 * channels ). The optional trkbuf
//   parameter can be used to get detailed status of the player. Put NULL/0 is unused.
// -------------------------------------------
...
static const short periodtable[] = {
    27392, 25856, 24384, 23040, 21696, 20480, 19328, 18240, 17216, 16256, 15360, 14496, 13696, 12928, 12192, 11520,
    10848, 10240, 9664,  9120,  8606,  8128,  7680,  7248,  6848,  6464,  6096,  5760,  5424,  5120,  4832,  4560,
    4304,  4064,  3840,  3624,  3424,  3232,  3048,  2880,  2712,  2560,  2416,  2280,  2152,  2032,  1920,  1812,
    1712,  1616,  1524,  1440,  1356,  1280,  1208,  1140,  1076,  1016,  960,   906,   856,   808,   762,   720,
    678,   640,   604,   570,   538,   508,   480,   453,   428,   404,   381,   360,   339,   320,   302,   285,
    269,   254,   240,   226,   214,   202,   190,   180,   170,   160,   151,   143,   135,   127,   120,   113,
    107,   101,   95,    90,    85,    80,    75,    71,    67,    63,    60,    56,    53,    50,    47,    45,
    42,    40,    37,    35,    33,    31,    30,    28,    27,    25,    24,    22,    21,    20,    19,    18,
    17,    16,    15,    14,    13,    13,    12,    11,    11,    10,    9,     9,     8,     8,     7,     7};

static const short sintable[] = {0,   24,  49,  74,  97,  120, 141, 161, 180, 197, 212, 224, 235, 244, 250, 253,
...
modtype modlist[] = {{"M!K!", 4},  {"M.K.", 4},  {"FLT4", 4},  {"FLT8", 8},  {"4CHN", 4},  {"6CHN", 6},  {"8CHN", 8},
                     {"10CH", 10}, {"12CH", 12}, {"14CH", 14}, {"16CH", 16}, {"18CH", 18}, {"20CH", 20}, {"22CH", 22},
                     {"24CH", 24}, {"26CH", 26}, {"28CH", 28}, {"30CH", 30}, {"32CH", 32}, {"", 0}};
...
bool jar_mod_setcfg(jar_mod_context_t* modctx, int samplerate, int bits, int stereo, int stereo_separation,
                    int filter) {
  if (modctx) {
          ...
          if (cptr->period != 0) {
            ...
            if (cptr->replen <= 2) {
              ...
              if ((cptr->samppos >> 10) >= (unsigned long)(cptr->replen + cptr->reppnt)) {
                cptr->samppos = ((unsigned long)(cptr->reppnt) << 10) +
                                (cptr->samppos % ((unsigned long)(cptr->replen + cptr->reppnt) << 10));
              }
            }
            ...
            if (trkbuf && !state_remaining_steps) {
              if (trkbuf->nb_of_state < trkbuf->nb_max_of_state) {
                ...
                trkbuf->track_state_buf[trkbuf->nb_of_state].cur_pattern_pos =
                    modctx->patternpos / modctx->number_of_channels;
