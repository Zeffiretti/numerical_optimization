...
static void terminate(void) {
  ...
  for (i = 0; i < _glfw.monitorCount; i++) {
    ...
    if (monitor->originalRamp.size) _glfwPlatformSetGammaRamp(monitor, &monitor->originalRamp);
...
GLFWAPI const char* glfwGetVersionString(void) { return _glfwPlatformGetVersionString(); }
