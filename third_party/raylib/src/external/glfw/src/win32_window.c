...
//
static void getFullWindowSize(DWORD style, DWORD exStyle, int contentWidth, int contentHeight, int* fullWidth,
...
static void applyAspectRatio(_GLFWwindow* window, int edge, RECT* area) {
  ...
  if (_glfwIsWindows10AnniversaryUpdateOrGreaterWin32()) dpi = GetDpiForWindow(window->win32.handle);

  getFullWindowSize(getWindowStyle(window), getWindowExStyle(window), 0, 0, &xoff, &yoff, dpi);
...
static int createNativeWindow(_GLFWwindow* window, const _GLFWwndconfig* wndconfig, const _GLFWfbconfig* fbconfig) {
  ...
  window->win32.handle =
      CreateWindowExW(exStyle, _GLFW_WNDCLASSNAME, wideTitle, style, xpos, ypos, fullWidth, fullHeight,
                      ...
                      NULL,  // No window menu
                      GetModuleHandleW(NULL), (LPVOID)wndconfig);
...
//
void _glfwUnregisterWindowClassWin32(void) { UnregisterClassW(_GLFW_WNDCLASSNAME, GetModuleHandleW(NULL)); }
...
void _glfwPlatformDestroyWindow(_GLFWwindow* window) {
  ...
  if (_glfw.win32.disabledCursorWindow == window) _glfw.win32.disabledCursorWindow = NULL;
...
void _glfwPlatformShowWindow(_GLFWwindow* window) { ShowWindow(window->win32.handle, SW_SHOWNA); }
...
int _glfwPlatformWindowHovered(_GLFWwindow* window) { return cursorInContentArea(window); }
...
void _glfwPlatformSetWindowResizable(_GLFWwindow* window, GLFWbool enabled) { updateWindowStyles(window); }

void _glfwPlatformSetWindowDecorated(_GLFWwindow* window, GLFWbool enabled) { updateWindowStyles(window); }
...
void _glfwPlatformPostEmptyEvent(void) { PostMessage(_glfw.win32.helperWindowHandle, WM_NULL, 0, 0); }
...
const char* _glfwPlatformGetScancodeName(int scancode) {
  if (scancode < 0 || scancode > (KF_EXTENDED | 0xff) || _glfw.win32.keycodes[scancode] == GLFW_KEY_UNKNOWN) {
...
EGLNativeDisplayType _glfwPlatformGetEGLNativeDisplay(void) { return GetDC(_glfw.win32.helperWindowHandle); }

EGLNativeWindowType _glfwPlatformGetEGLNativeWindow(_GLFWwindow* window) { return window->win32.handle; }
...
VkResult _glfwPlatformCreateWindowSurface(VkInstance instance, _GLFWwindow* window,
  ...
  if (err) {
    _glfwInputError(GLFW_PLATFORM_ERROR, "Win32: Failed to create Vulkan surface: %s", _glfwGetVulkanResultString(err));
  }
