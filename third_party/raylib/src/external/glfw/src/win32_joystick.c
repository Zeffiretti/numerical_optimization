...
static BOOL CALLBACK deviceCallback(const DIDEVICEINSTANCE* di, void* user) {
  ...
  data.device = device;
  data.objects = calloc(dc.dwAxes + (size_t)dc.dwButtons + dc.dwPOVs, sizeof(_GLFWjoyobjectWin32));
  ...
  if (memcmp(&di->guidProduct.Data4[2], "PIDVID", 6) == 0) {
    ...
    sprintf(guid, "05000000%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x00", name[0], name[1], name[2], name[3], name[4],
...
void _glfwDetectJoystickConnectionWin32(void) {
  if (_glfw.win32.xinput.instance) {
    ...
    for (index = 0; index < XUSER_MAX_COUNT; index++) {
      ...
      for (jid = 0; jid <= GLFW_JOYSTICK_LAST; jid++) {
        if (_glfw.joysticks[jid].present && _glfw.joysticks[jid].win32.device == NULL &&
...
void _glfwDetectJoystickDisconnectionWin32(void) {
  ...
  for (jid = 0; jid <= GLFW_JOYSTICK_LAST; jid++) {
    ...
    if (js->present) _glfwPlatformPollJoystick(js, _GLFW_POLL_PRESENCE);
  }
}
...
GLFWbool _glfwPlatformInitJoysticks(void) {
  if (_glfw.win32.dinput8.instance) {
    if (FAILED(DirectInput8Create(GetModuleHandle(NULL), DIRECTINPUT_VERSION, &IID_IDirectInput8W,
...
void _glfwPlatformTerminateJoysticks(void) {
  ...
  for (jid = GLFW_JOYSTICK_1; jid <= GLFW_JOYSTICK_LAST; jid++) closeJoystick(_glfw.joysticks + jid);
