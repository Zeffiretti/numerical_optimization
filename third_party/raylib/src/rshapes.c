...
void DrawLineBezierCubic(Vector2 startPos, Vector2 endPos, Vector2 startControlPos, Vector2 endControlPos, float thick,
                         Color color) {
  const float step = 1.0f / BEZIER_LINE_DIVISIONS;
...
void DrawCircleSector(Vector2 center, float radius, float startAngle, float endAngle, int segments, Color color) {
  ...
  for (int i = 0; i < segments / 2; i++) {
    ...
    rlVertex2f(center.x + sinf(DEG2RAD * (angle + stepLength * 2)) * radius,
...
void DrawCircleSectorLines(Vector2 center, float radius, float startAngle, float endAngle, int segments, Color color) {
  ...
  if ((int)(endAngle - startAngle) % 360 == 0) {
    limit = 2 * segments;
    showCapLines = false;
  }
...
void DrawCircleV(Vector2 center, float radius, Color color) { DrawCircleSector(center, radius, 0, 360, 36, color); }
...
void DrawEllipse(int centerX, int centerY, float radiusH, float radiusV, Color color) {
  ...
  for (int i = 0; i < 360; i += 10) {
    ...
    rlVertex2f((float)centerX + sinf(DEG2RAD * (i + 10)) * radiusH,
               (float)centerY + cosf(DEG2RAD * (i + 10)) * radiusV);
  }
  rlEnd();
}
...
void DrawRing(Vector2 center, float innerRadius, float outerRadius, float startAngle, float endAngle, int segments,
  ...
  for (int i = 0; i < segments; i++) {
    ...
    rlTexCoord2f((texShapesRec.x + texShapesRec.width) / texShapes.width,
                 (texShapesRec.y + texShapesRec.height) / texShapes.height);
    rlVertex2f(center.x + sinf(DEG2RAD * (angle + stepLength)) * outerRadius,
               center.y + cosf(DEG2RAD * (angle + stepLength)) * outerRadius);

    rlTexCoord2f((texShapesRec.x + texShapesRec.width) / texShapes.width, texShapesRec.y / texShapes.height);
    rlVertex2f(center.x + sinf(DEG2RAD * (angle + stepLength)) * innerRadius,
...
void DrawRingLines(Vector2 center, float innerRadius, float outerRadius, float startAngle, float endAngle, int segments,
                   Color color) {
  if (startAngle == endAngle) return;
...
// Draw a color-filled rectangle
void DrawRectangleRec(Rectangle rec, Color color) { DrawRectanglePro(rec, (Vector2){0.0f, 0.0f}, 0.0f, color); }
...
void DrawRectangleGradientV(int posX, int posY, int width, int height, Color color1, Color color2) {
  DrawRectangleGradientEx((Rectangle){(float)posX, (float)posY, (float)width, (float)height}, color1, color2, color2,
                          color1);
}
...
void DrawRectangleGradientH(int posX, int posY, int width, int height, Color color1, Color color2) {
  DrawRectangleGradientEx((Rectangle){(float)posX, (float)posY, (float)width, (float)height}, color1, color1, color2,
                          color2);
}
...
void DrawRectangleRounded(Rectangle rec, float roundness, int segments, Color color) {
  ...
  */
  ...
  rlBegin(RL_QUADS);
  // Draw all of the 4 corners: [1] Upper Left Corner, [3] Upper Right Corner, [5] Lower Right Corner, [7] Lower Left
  // Corner
  ...
  rlSetTexture(0);
#else
  rlCheckRenderBatchLimit(12 * segments +
                          5 * 6);  // 4 corners with 3 vertices per segment + 5 rectangles with 6 vertices each
...
void DrawRectangleRoundedLines(Rectangle rec, float roundness, int segments, float lineThick, Color color) {
  ...
  */
  const Vector2 point[16] = {
      ...
      {rec.x + rec.width + lineThick, (float)rec.y + innerRadius},  // PO, P1, P2
  ...
  const Vector2 centers[4] = {
      ...
      {(float)(rec.x + rec.width) - innerRadius, (float)rec.y + innerRadius},  // P16, P17
  ...
  if (lineThick > 1) {
#if defined(SUPPORT_QUADS_DRAW_MODE)
    rlCheckRenderBatchLimit(4 * 4 * segments +
                            4 * 4);  // 4 corners with 4 vertices for each segment + 4 rectangles with 4 vertices each
    ...
    rlCheckRenderBatchLimit(
        4 * 6 * segments +
        4 * 6);  // 4 corners with 6(2*3) vertices for each segment + 4 rectangles with 6 vertices each
    ...
    rlCheckRenderBatchLimit(8 * segments +
                            4 * 2);  // 4 corners with 2 vertices for each segment + 4 rectangles with 2 vertices each
...
bool CheckCollisionLines(Vector2 startPos1, Vector2 endPos1, Vector2 startPos2, Vector2 endPos2,
                         Vector2* collisionPoint) {
  const float div =
      (endPos2.y - startPos2.y) * (endPos1.x - startPos1.x) - (endPos2.x - startPos2.x) * (endPos1.y - startPos1.y);

  if (fabsf(div) < FLT_EPSILON) return false;

  const float xi = ((startPos2.x - endPos2.x) * (startPos1.x * endPos1.y - startPos1.y * endPos1.x) -
                    (startPos1.x - endPos1.x) * (startPos2.x * endPos2.y - startPos2.y * endPos2.x)) /
                   div;
  const float yi = ((startPos2.y - endPos2.y) * (startPos1.x * endPos1.y - startPos1.y * endPos1.x) -
  ...
  if (fabsf(startPos1.x - endPos1.x) > FLT_EPSILON &&
    ...
    return false;
  if (fabsf(startPos2.x - endPos2.x) > FLT_EPSILON &&
      (xi < fminf(startPos2.x, endPos2.x) || xi > fmaxf(startPos2.x, endPos2.x)))
    return false;
  if (fabsf(startPos1.y - endPos1.y) > FLT_EPSILON &&
      (yi < fminf(startPos1.y, endPos1.y) || yi > fmaxf(startPos1.y, endPos1.y)))
    return false;
  if (fabsf(startPos2.y - endPos2.y) > FLT_EPSILON &&
