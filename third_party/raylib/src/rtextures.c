...
#if defined(SUPPORT_FILEFORMAT_PNG) || defined(SUPPORT_FILEFORMAT_BMP) || defined(SUPPORT_FILEFORMAT_TGA) || \
    defined(SUPPORT_FILEFORMAT_JPG) || defined(SUPPORT_FILEFORMAT_GIF) || defined(SUPPORT_FILEFORMAT_PIC) || \
    defined(SUPPORT_FILEFORMAT_HDR) || defined(SUPPORT_FILEFORMAT_PSD)
...
Image LoadImageAnim(const char* fileName, int* frames) {
  ...
  if (IsFileExtension(fileName, ".gif")) {
    ...
    if (fileData != NULL) {
      ...
      int* delays = NULL;
      image.data =
          stbi_load_gif_from_memory(fileData, dataSize, &delays, &image.width, &image.height, &frameCount, &comp, 4);
...
#endif
#if defined(SUPPORT_FILEFORMAT_JPG)
      || (TextIsEqual(fileExtLower, ".jpg") || TextIsEqual(fileExtLower, ".jpeg"))
#endif
#if defined(SUPPORT_FILEFORMAT_GIF)
...
#  endif
...
#endif
#if defined(SUPPORT_FILEFORMAT_DDS)
  else if (TextIsEqual(fileExtLower, ".dds"))
    image = LoadDDS(fileData, dataSize);
#endif
#if defined(SUPPORT_FILEFORMAT_PKM)
  else if (TextIsEqual(fileExtLower, ".pkm"))
    image = LoadPKM(fileData, dataSize);
#endif
#if defined(SUPPORT_FILEFORMAT_KTX)
  else if (TextIsEqual(fileExtLower, ".ktx"))
    image = LoadKTX(fileData, dataSize);
#endif
#if defined(SUPPORT_FILEFORMAT_PVR)
  else if (TextIsEqual(fileExtLower, ".pvr"))
    image = LoadPVR(fileData, dataSize);
#endif
#if defined(SUPPORT_FILEFORMAT_ASTC)
  else if (TextIsEqual(fileExtLower, ".astc"))
  ...
  if (image.data != NULL)
    TRACELOG(LOG_INFO, "IMAGE: Data loaded successfully (%ix%i | %s | %i mipmaps)", image.width, image.height,
             rlGetPixelFormatName(image.format), image.mipmaps);
  else
...
Image LoadImageFromTexture(Texture2D texture) {
  ...
  if (texture.format < PIXELFORMAT_COMPRESSED_DXT1_RGB) {
    ...
    if (image.data != NULL) {
      ...
      TRACELOG(LOG_WARNING, "TEXTURE: [ID %i] Failed to retrieve pixel data", texture.id);
  } else
...
void UnloadImage(Image image) { RL_FREE(image.data); }
...
#  if defined(SUPPORT_FILEFORMAT_PNG)
  if (IsFileExtension(fileName, ".png")) {
    int dataSize = 0;
    unsigned char* fileData = stbi_write_png_to_mem((const unsigned char*)imgData, image.width * channels, image.width,
                                                    image.height, channels, &dataSize);
...
#  endif
  ...
  if (success != 0)
    TRACELOG(LOG_INFO, "FILEIO: [%s] Image exported successfully", fileName);
  else
    TRACELOG(LOG_WARNING, "FILEIO: [%s] Failed to export image", fileName);
...
Image GenImageColor(int width, int height, Color color) {
  ...
  Image image = {
      .data = pixels, .width = width, .height = height, .format = PIXELFORMAT_UNCOMPRESSED_R8G8B8A8, .mipmaps = 1};
...
void ImageCrop(Image* image, Rectangle crop) {
  ...
  if (image->format >= PIXELFORMAT_COMPRESSED_DXT1_RGB)
    ...
    for (int y = (int)crop.y, offsetSize = 0; y < (int)(crop.y + crop.height); y++) {
      memcpy(croppedData + offsetSize, ((unsigned char*)image->data) + (y * image->width + (int)crop.x) * bytesPerPixel,
             (int)crop.width * bytesPerPixel);
    ...
    for (int y = (int)crop.y; y < (int)(crop.y + crop.height); y++)
    {
        ...
        {
            //memcpy(croppedData + ((y - (int)crop.y)*(int)crop.width + (x - (int)crop.x))*bytesPerPixel, ((unsigned
    char *)image->data) + (y*image->width + x)*bytesPerPixel, bytesPerPixel); for (int i = 0; i < bytesPerPixel; i++)
    croppedData[((y - (int)crop.y)*(int)crop.width + (x - (int)crop.x))*bytesPerPixel + i] = ((unsigned char
    *)image->data)[(y*image->width + x)*bytesPerPixel + i];
        }
    }
    */
      ...
      switch (image->format) {
        ...
        case PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA: {
          ...
          for (int i = 0; i < image->width * image->height * 2; i += 2, k++) {
            ((unsigned char*)image->data)[i] =
                (unsigned char)((pixels[k].x * 0.299f + (float)pixels[k].y * 0.587f + (float)pixels[k].z * 0.114f) *
                                255.0f);
        ...
        case PIXELFORMAT_UNCOMPRESSED_R5G5B5A1: {
          ...
          for (int i = 0; i < image->width * image->height; i++) {
            ...
            ((unsigned short*)image->data)[i] =
                (unsigned short)r << 11 | (unsigned short)g << 6 | (unsigned short)b << 1 | (unsigned short)a;
          }
        ...
        case PIXELFORMAT_UNCOMPRESSED_R4G4B4A4: {
          ...
          for (int i = 0; i < image->width * image->height; i++) {
            ...
            ((unsigned short*)image->data)[i] =
                (unsigned short)r << 12 | (unsigned short)g << 8 | (unsigned short)b << 4 | (unsigned short)a;
          }
  ...
  for (int i = 0; i < size; i++) {
    ...
    if (codepoint == '\n') {
      ...
      if ((codepoint != ' ') && (codepoint != '\t')) {
        Rectangle rec = {(float)(textOffsetX + font.glyphs[index].offsetX),
                         (float)(textOffsetY + font.glyphs[index].offsetY), (float)font.recs[index].width,
                         (float)font.recs[index].height};
        ImageDraw(&imText, font.glyphs[index].image,
                  (Rectangle){0, 0, (float)font.glyphs[index].image.width, (float)font.glyphs[index].image.height}, rec,
                  tint);
      }
...
void ImageResize(Image* image, int newWidth, int newHeight) {
  ...
  if (fastPath) {
    ...
    switch (image->format) {
      case PIXELFORMAT_UNCOMPRESSED_GRAYSCALE:
        stbir_resize_uint8((unsigned char*)image->data, image->width, image->height, 0, output, newWidth, newHeight, 0,
                           1);
        break;
      case PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA:
        stbir_resize_uint8((unsigned char*)image->data, image->width, image->height, 0, output, newWidth, newHeight, 0,
    ...
    stbir_resize_uint8((unsigned char*)pixels, image->width, image->height, 0, (unsigned char*)output, newWidth,
                       newHeight, 0, 4);
...
void ImageResizeCanvas(Image* image, int newWidth, int newHeight, int offsetX, int offsetY, Color fill) {
  ...
  if (image->format >= PIXELFORMAT_COMPRESSED_DXT1_RGB)
    ...
    for (int y = 0; y < (int)srcRec.height; y++) {
      memcpy(resizedData + dstOffsetSize,
             ((unsigned char*)image->data) + ((y + (int)srcRec.y) * image->width + (int)srcRec.x) * bytesPerPixel,
...
void ImageMipmaps(Image* image) {
  ...
  if (image->mipmaps < mipCount) {
    ...
    for (int i = 1; i < mipCount; i++) {
      TRACELOGD("IMAGE: Generating mipmap level: %i (%i x %i) - size: %i - offset: 0x%x", i, mipWidth, mipHeight,
                mipSize, nextmip);
...
void ImageDither(Image* image, int rBpp, int gBpp, int bBpp, int aBpp) {
  ...
  if ((rBpp + gBpp + bBpp + aBpp) > 16) {
    TRACELOG(LOG_WARNING, "IMAGE: Unsupported dithering bpps (%ibpp), only 16bpp or lower modes supported",
             (rBpp + gBpp + bBpp + aBpp));
  } else {
    Color* pixels = LoadImageColors(*image);
    ...
    if ((rBpp == 5) && (gBpp == 6) && (bBpp == 5) && (aBpp == 0))
      ...
      image->format = 0;
      TRACELOG(LOG_WARNING, "IMAGE: Unsupported dithered OpenGL internal format: %ibpp (R%iG%iB%iA%i)",
               (rBpp + gBpp + bBpp + aBpp), rBpp, gBpp, bBpp, aBpp);
    }
      ...
      for (int x = 0; x < image->width; x++) {
        ...
        if (x < (image->width - 1)) {
          pixels[y * image->width + x + 1].r =
              MIN((int)pixels[y * image->width + x + 1].r + (int)((float)rError * 7.0f / 16), 0xff);
          pixels[y * image->width + x + 1].g =
              MIN((int)pixels[y * image->width + x + 1].g + (int)((float)gError * 7.0f / 16), 0xff);
          pixels[y * image->width + x + 1].b =
              MIN((int)pixels[y * image->width + x + 1].b + (int)((float)bError * 7.0f / 16), 0xff);
        }

        if ((x > 0) && (y < (image->height - 1))) {
          pixels[(y + 1) * image->width + x - 1].r =
              MIN((int)pixels[(y + 1) * image->width + x - 1].r + (int)((float)rError * 3.0f / 16), 0xff);
          pixels[(y + 1) * image->width + x - 1].g =
              MIN((int)pixels[(y + 1) * image->width + x - 1].g + (int)((float)gError * 3.0f / 16), 0xff);
          pixels[(y + 1) * image->width + x - 1].b =
              MIN((int)pixels[(y + 1) * image->width + x - 1].b + (int)((float)bError * 3.0f / 16), 0xff);
        }

        if (y < (image->height - 1)) {
          pixels[(y + 1) * image->width + x].r =
              MIN((int)pixels[(y + 1) * image->width + x].r + (int)((float)rError * 5.0f / 16), 0xff);
          pixels[(y + 1) * image->width + x].g =
              MIN((int)pixels[(y + 1) * image->width + x].g + (int)((float)gError * 5.0f / 16), 0xff);
          pixels[(y + 1) * image->width + x].b =
              MIN((int)pixels[(y + 1) * image->width + x].b + (int)((float)bError * 5.0f / 16), 0xff);
        }

        if ((x < (image->width - 1)) && (y < (image->height - 1))) {
          pixels[(y + 1) * image->width + x + 1].r =
              MIN((int)pixels[(y + 1) * image->width + x + 1].r + (int)((float)rError * 1.0f / 16), 0xff);
          pixels[(y + 1) * image->width + x + 1].g =
              MIN((int)pixels[(y + 1) * image->width + x + 1].g + (int)((float)gError * 1.0f / 16), 0xff);
          pixels[(y + 1) * image->width + x + 1].b =
              MIN((int)pixels[(y + 1) * image->width + x + 1].b + (int)((float)bError * 1.0f / 16), 0xff);
        }
        ...
        ((unsigned short*)image->data)[y * image->width + x] =
            (rPixel << (gBpp + bBpp + aBpp)) | (gPixel << (bBpp + aBpp)) | (bPixel << aBpp) | aPixel;
      }
    }
...
void ImageFlipVertical(Image* image) {
  ...
  if (image->format >= PIXELFORMAT_COMPRESSED_DXT1_RGB)
    ...
    for (int i = (image->height - 1), offsetSize = 0; i >= 0; i--) {
      memcpy(flippedData + offsetSize, ((unsigned char*)image->data) + i * image->width * bytesPerPixel,
  ...
  if (image->format >= PIXELFORMAT_COMPRESSED_DXT1_RGB)
    ...
    for (int y = 0; y < image->height; y++) {
      for (int x = 0; x < image->width; x++) {
        ...
        // memcpy(flippedData + (y*image->width + x)*bytesPerPixel, ((unsigned char *)image->data) + (y*image->width +
        // (image->width - 1 - x))*bytesPerPixel, bytesPerPixel);

        // OPTION 2: Just copy data pixel by pixel
        for (int i = 0; i < bytesPerPixel; i++)
          flippedData[(y * image->width + x) * bytesPerPixel + i] =
              ((unsigned char*)image->data)[(y * image->width + (image->width - 1 - x)) * bytesPerPixel + i];
      }
    }
...
void ImageRotateCW(Image* image) {
  ...
  if (image->format >= PIXELFORMAT_COMPRESSED_DXT1_RGB)
    ...
    for (int y = 0; y < image->height; y++) {
      for (int x = 0; x < image->width; x++) {
        // memcpy(rotatedData + (x*image->height + (image->height - y - 1))*bytesPerPixel, ((unsigned char
        // *)image->data) + (y*image->width + x)*bytesPerPixel, bytesPerPixel);
        for (int i = 0; i < bytesPerPixel; i++)
          rotatedData[(x * image->height + (image->height - y - 1)) * bytesPerPixel + i] =
              ((unsigned char*)image->data)[(y * image->width + x) * bytesPerPixel + i];
      }
    }
...
void ImageRotateCCW(Image* image) {
  ...
  if (image->format >= PIXELFORMAT_COMPRESSED_DXT1_RGB)
    ...
    for (int y = 0; y < image->height; y++) {
      for (int x = 0; x < image->width; x++) {
        // memcpy(rotatedData + (x*image->height + y))*bytesPerPixel, ((unsigned char *)image->data) + (y*image->width +
        // (image->width - x - 1))*bytesPerPixel, bytesPerPixel);
        for (int i = 0; i < bytesPerPixel; i++)
          rotatedData[(x * image->height + y) * bytesPerPixel + i] =
              ((unsigned char*)image->data)[(y * image->width + (image->width - x - 1)) * bytesPerPixel + i];
      }
    }
...
void ImageColorGrayscale(Image* image) { ImageFormat(image, PIXELFORMAT_UNCOMPRESSED_GRAYSCALE); }
...
void ImageColorReplace(Image* image, Color color, Color replace) {
  ...
  for (int y = 0; y < image->height; y++) {
    for (int x = 0; x < image->width; x++) {
      if ((pixels[y * image->width + x].r == color.r) && (pixels[y * image->width + x].g == color.g) &&
          (pixels[y * image->width + x].b == color.b) && (pixels[y * image->width + x].a == color.a)) {
...
void UnloadImageColors(Color* colors) { RL_FREE(colors); }

// Unload colors palette loaded with LoadImagePalette()
void UnloadImagePalette(Color* colors) { RL_FREE(colors); }
...
Color GetImageColor(Image image, int x, int y) {
  ...
  if ((x >= 0) && (x < image.width) && (y >= 0) && (y < image.height)) {
    ...
    TRACELOG(LOG_WARNING, "Requested image pixel (%i, %i) out of bounds", x, y);
...
void ImageDrawPixel(Image* dst, int x, int y, Color color) {
  ...
  switch (dst->format) {
    ...
    case PIXELFORMAT_UNCOMPRESSED_R5G6B5: {
      ...
      ((unsigned short*)dst->data)[y * dst->width + x] =
          (unsigned short)r << 11 | (unsigned short)g << 5 | (unsigned short)b;

    } break;
    case PIXELFORMAT_UNCOMPRESSED_R5G5B5A1: {
      // NOTE: Calculate R5G5B5A1 equivalent color
      Vector4 coln = {(float)color.r / 255.0f, (float)color.g / 255.0f, (float)color.b / 255.0f,
...
void ImageDrawRectangleLines(Image* dst, Rectangle rec, int thick, Color color) {
  ...
  ImageDrawRectangle(dst, (int)(rec.x + rec.width - thick), (int)(rec.y + thick), thick, (int)(rec.height - thick * 2),
                     color);
...
void ImageDraw(Image* dst, Image src, Rectangle srcRec, Rectangle dstRec, Color tint) {
  // Security check to avoid program crash
  if ((dst->data == NULL) || (dst->width == 0) || (dst->height == 0) || (src.data == NULL) || (src.width == 0) ||
      (src.height == 0))
    return;
  ...
  if (dst->format >= PIXELFORMAT_COMPRESSED_DXT1_RGB)
    ...
    if ((tint.a == 255) &&
        ((srcPtr->format == PIXELFORMAT_UNCOMPRESSED_GRAYSCALE) ||
         (srcPtr->format == PIXELFORMAT_UNCOMPRESSED_R8G8B8) || (srcPtr->format == PIXELFORMAT_UNCOMPRESSED_R5G6B5)))
      blendRequired = false;
    ...
    unsigned char* pSrcBase =
        (unsigned char*)srcPtr->data + ((int)srcRec.y * srcPtr->width + (int)srcRec.x) * bytesPerPixelSrc;
    unsigned char* pDstBase =
        (unsigned char*)dst->data + ((int)dstRec.y * dst->width + (int)dstRec.x) * bytesPerPixelDst;
...
void ImageDrawTextEx(Image* dst, Font font, const char* text, Vector2 position, float fontSize, float spacing,
                     Color tint) {
...
TextureCubemap LoadTextureCubemap(Image image, int layout) {
  ...
  if (layout != CUBEMAP_LAYOUT_AUTO_DETECT) {
    ...
    for (int i = 0; i < 6; i++)
      ImageDraw(&faces, image, faceRecs[i], (Rectangle){0, (float)size * i, (float)size, (float)size}, WHITE);
...
void SetTextureFilter(Texture2D texture, int filter) {
  switch (filter) {
    ...
    case TEXTURE_FILTER_BILINEAR: {
      if (texture.mipmaps > 1) {
        // RL_TEXTURE_FILTER_LINEAR_MIP_NEAREST - tex filter: BILINEAR, mipmaps filter: POINT (sharp switching between
        // mipmaps) Alternative: RL_TEXTURE_FILTER_NEAREST_MIP_LINEAR - tex filter: POINT, mipmaps filter: BILINEAR
        // (smooth transition between mipmaps)
    ...
    case TEXTURE_FILTER_TRILINEAR: {
      if (texture.mipmaps > 1) {
        // RL_TEXTURE_FILTER_MIP_LINEAR - tex filter: BILINEAR, mipmaps filter: BILINEAR (smooth transition between
        // mipmaps)
...
void DrawTextureV(Texture2D texture, Vector2 position, Color tint) { DrawTextureEx(texture, position, 0, 1.0f, tint); }
...
void DrawTextureTiled(Texture2D texture, Rectangle source, Rectangle dest, Vector2 origin, float rotation, float scale,
                      Color tint) {
  ...
  if ((dest.width < tileWidth) && (dest.height < tileHeight)) {
    ...
    DrawTexturePro(texture,
                   (Rectangle){source.x, source.y, ((float)dest.width / tileWidth) * source.width,
    ...
    for (; dy + tileHeight < dest.height; dy += tileHeight) {
      DrawTexturePro(texture,
                     (Rectangle){source.x, source.y, ((float)dest.width / tileWidth) * source.width, source.height},
                     (Rectangle){dest.x, dest.y + dy, dest.width, (float)tileHeight}, origin, rotation, tint);
    }
    ...
    for (; dx + tileWidth < dest.width; dx += tileWidth) {
      DrawTexturePro(texture,
                     (Rectangle){source.x, source.y, source.width, ((float)dest.height / tileHeight) * source.height},
                     (Rectangle){dest.x + dx, dest.y, (float)tileWidth, dest.height}, origin, rotation, tint);
    }

    // Fit last tile
    if (dx < dest.width) {
      DrawTexturePro(texture,
                     (Rectangle){source.x, source.y, ((float)(dest.width - dx) / tileWidth) * source.width,
                                 ((float)dest.height / tileHeight) * source.height},
    ...
    for (; dx + tileWidth < dest.width; dx += tileWidth) {
      int dy = 0;
      for (; dy + tileHeight < dest.height; dy += tileHeight) {
        DrawTexturePro(texture, source, (Rectangle){dest.x + dx, dest.y + dy, (float)tileWidth, (float)tileHeight},
      ...
      if (dy < dest.height) {
        DrawTexturePro(
            texture,
            (Rectangle){source.x, source.y, source.width, ((float)(dest.height - dy) / tileHeight) * source.height},
    ...
    if (dx < dest.width) {
      ...
      for (; dy + tileHeight < dest.height; dy += tileHeight) {
        DrawTexturePro(
            texture,
            (Rectangle){source.x, source.y, ((float)(dest.width - dx) / tileWidth) * source.width, source.height},
      ...
      if (dy < dest.height) {
        DrawTexturePro(texture,
                       (Rectangle){source.x, source.y, ((float)(dest.width - dx) / tileWidth) * source.width,
                                   ((float)(dest.height - dy) / tileHeight) * source.height},
                       (Rectangle){dest.x + dx, dest.y + dy, dest.width - dx, dest.height - dy}, origin, rotation,
                       tint);
      }
    }
  }
...
void DrawTextureNPatch(Texture2D texture, NPatchInfo nPatchInfo, Rectangle dest, Vector2 origin, float rotation,
                       Color tint) {
  if (texture.id > 0) {
    float width = (float)texture.width;
    float height = (float)texture.height;
...
#define COLORALPHABLEND_INTEGERS
  ...
  if (src.a == 0)
    ...
    unsigned int alpha =
        (unsigned int)src.a + 1;  // We are shifting by 8 (dividing by 256), so we need to take that excess into account
    out.a = (unsigned char)(((unsigned int)alpha * 256 + (unsigned int)dst.a * (256 - alpha)) >> 8);

    if (out.a > 0) {
      out.r = (unsigned char)((((unsigned int)src.r * alpha * 256 +
                                (unsigned int)dst.r * (unsigned int)dst.a * (256 - alpha)) /
                               out.a) >>
                              8);
      out.g = (unsigned char)((((unsigned int)src.g * alpha * 256 +
                                (unsigned int)dst.g * (unsigned int)dst.a * (256 - alpha)) /
                               out.a) >>
                              8);
      out.b = (unsigned char)((((unsigned int)src.b * alpha * 256 +
  ...
  if (src.a == 0)
    ...
    out = (Color){(unsigned char)(fout.x * 255.0f), (unsigned char)(fout.y * 255.0f), (unsigned char)(fout.z * 255.0f),
...
Color GetPixelColor(void* srcPtr, int format) {
  ...
  switch (format) {
    case PIXELFORMAT_UNCOMPRESSED_GRAYSCALE:
      color = (Color){((unsigned char*)srcPtr)[0], ((unsigned char*)srcPtr)[0], ((unsigned char*)srcPtr)[0], 255};
      break;
    case PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA:
      color = (Color){((unsigned char*)srcPtr)[0], ((unsigned char*)srcPtr)[0], ((unsigned char*)srcPtr)[0],
    ...
    case PIXELFORMAT_UNCOMPRESSED_R8G8B8A8:
      color = (Color){((unsigned char*)srcPtr)[0], ((unsigned char*)srcPtr)[1], ((unsigned char*)srcPtr)[2],
                      ((unsigned char*)srcPtr)[3]};
      break;
    case PIXELFORMAT_UNCOMPRESSED_R8G8B8:
      color = (Color){((unsigned char*)srcPtr)[0], ((unsigned char*)srcPtr)[1], ((unsigned char*)srcPtr)[2], 255};
      break;
    case PIXELFORMAT_UNCOMPRESSED_R32: {
...
#  define FOURCC_DXT5 0x35545844  // Equivalent to "DXT5" in ASCII
  ...
  if (fileDataPtr != NULL) {
    ...
    if ((ddsHeaderId[0] != 'D') || (ddsHeaderId[1] != 'D') || (ddsHeaderId[2] != 'S') || (ddsHeaderId[3] != ' ')) {
      ...
      if (ddsHeader->mipmapCount == 0)
        image.mipmaps = 1;  // Parameter not used
      else
      ...
      } else if (ddsHeader->ddspf.flags == 0x41 && ddsHeader->ddspf.rgbBitCount == 32)  // DDS_RGBA, no compressed
      {
        ...
        image.format = PIXELFORMAT_UNCOMPRESSED_R8G8B8A8;
      } else if (((ddsHeader->ddspf.flags == 0x04) || (ddsHeader->ddspf.flags == 0x05)) &&
                 (ddsHeader->ddspf.fourCC > 0))  // Compressed
      {
        int dataSize = 0;

        // Calculate data size, including all mipmaps
        if (ddsHeader->mipmapCount > 1)
          dataSize = ddsHeader->pitchOrLinearSize * 2;
        else
          dataSize = ddsHeader->pitchOrLinearSize;
...
static Image LoadPKM(const unsigned char* fileData, unsigned int fileSize) {
  ...
  // version 20: format: 0=ETC1_RGB, 1=ETC2_RGB, 2=ETC2_RGBA_OLD, 3=ETC2_RGBA, 4=ETC2_RGBA1, 5=ETC2_R, 6=ETC2_RG,
  // 7=ETC2_SIGNED_R, 8=ETC2_SIGNED_R
  ...
  if (fileDataPtr != NULL) {
    ...
    if ((pkmHeader->id[0] != 'P') || (pkmHeader->id[1] != 'K') || (pkmHeader->id[2] != 'M') ||
        (pkmHeader->id[3] != ' ')) {
...
static Image LoadKTX(const unsigned char* fileData, unsigned int fileSize) {
  ...
  if (fileDataPtr != NULL) {
    ...
    if ((ktxHeader->id[1] != 'K') || (ktxHeader->id[2] != 'T') || (ktxHeader->id[3] != 'X') ||
      ...
      if (ktxHeader->glInternalFormat == 0x8D64)
        image.format = PIXELFORMAT_COMPRESSED_ETC1_RGB;
      else if (ktxHeader->glInternalFormat == 0x9274)
        image.format = PIXELFORMAT_COMPRESSED_ETC2_RGB;
      else if (ktxHeader->glInternalFormat == 0x9278)
        image.format = PIXELFORMAT_COMPRESSED_ETC2_EAC_RGBA;
    }
  }
...
static int SaveKTX(Image image, const char* fileName) {
  ...
  typedef struct {
    ...
    unsigned int faces;                 // Cubemap faces, for no-cubemap = 1
    unsigned int mipmapLevels;          // Non-mipmapped textures = 1
    unsigned int keyValueDataSize;      // Used to encode any arbitrary data...         // KTX 2.0: UInt32 levelOrder -
                                    // ordering of the mipmap levels, usually 0 KTX 2.0: UInt32 supercompressionScheme -
                                    // 0 (None), 1 (Crunch CRN), 2 (Zlib DEFLATE)...
  ...
  ktxHeader.faces = 1;
  ktxHeader.mipmapLevels =
      image.mipmaps;  // If it was 0, it means mipmaps should be generated on loading (not for compressed formats)
  ktxHeader.keyValueDataSize = 0;  // No extra data after the header

  rlGetGlTextureFormats(image.format, &ktxHeader.glInternalFormat, &ktxHeader.glFormat,
                        &ktxHeader.glType);             // rlgl module function
  ktxHeader.glBaseInternalFormat = ktxHeader.glFormat;  // KTX 1.1 only

  // NOTE: We can save into a .ktx all PixelFormats supported by raylib, including compressed formats like DXT, ETC or
  // ASTC

  if (ktxHeader.glFormat == -1)
    TRACELOG(LOG_WARNING, "IMAGE: GL format not supported for KTX export (%i)", ktxHeader.glFormat);
  else {
...
#  endif
  ...
  if (fileDataPtr != NULL) {
    ...
    if (pvrVersion == 0x50) {
      ...
      if ((pvrHeader->id[0] != 'P') || (pvrHeader->id[1] != 'V') || (pvrHeader->id[2] != 'R') ||
          (pvrHeader->id[3] != 3)) {
        ...
        if (((pvrHeader->channels[0] == 'l') && (pvrHeader->channels[1] == 0)) && (pvrHeader->channelDepth[0] == 8))
          image.format = PIXELFORMAT_UNCOMPRESSED_GRAYSCALE;
        else if (((pvrHeader->channels[0] == 'l') && (pvrHeader->channels[1] == 'a')) &&
                 ((pvrHeader->channelDepth[0] == 8) && (pvrHeader->channelDepth[1] == 8)))
          image.format = PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA;
        else if ((pvrHeader->channels[0] == 'r') && (pvrHeader->channels[1] == 'g') &&
                 (pvrHeader->channels[2] == 'b')) {
          if (pvrHeader->channels[3] == 'a') {
            if ((pvrHeader->channelDepth[0] == 5) && (pvrHeader->channelDepth[1] == 5) &&
                (pvrHeader->channelDepth[2] == 5) && (pvrHeader->channelDepth[3] == 1))
              image.format = PIXELFORMAT_UNCOMPRESSED_R5G5B5A1;
            else if ((pvrHeader->channelDepth[0] == 4) && (pvrHeader->channelDepth[1] == 4) &&
                     (pvrHeader->channelDepth[2] == 4) && (pvrHeader->channelDepth[3] == 4))
              image.format = PIXELFORMAT_UNCOMPRESSED_R4G4B4A4;
            else if ((pvrHeader->channelDepth[0] == 8) && (pvrHeader->channelDepth[1] == 8) &&
                     (pvrHeader->channelDepth[2] == 8) && (pvrHeader->channelDepth[3] == 8))
              image.format = PIXELFORMAT_UNCOMPRESSED_R8G8B8A8;
          } else if (pvrHeader->channels[3] == 0) {
            if ((pvrHeader->channelDepth[0] == 5) && (pvrHeader->channelDepth[1] == 6) &&
                (pvrHeader->channelDepth[2] == 5))
              image.format = PIXELFORMAT_UNCOMPRESSED_R5G6B5;
            else if ((pvrHeader->channelDepth[0] == 8) && (pvrHeader->channelDepth[1] == 8) &&
                     (pvrHeader->channelDepth[2] == 8))
              image.format = PIXELFORMAT_UNCOMPRESSED_R8G8B8;
          }
        } else if (pvrHeader->channels[0] == 2)
          image.format = PIXELFORMAT_COMPRESSED_PVRT_RGB;
        else if (pvrHeader->channels[0] == 3)
    ...
    } else if (pvrVersion == 52)
      TRACELOG(LOG_INFO, "IMAGE: PVRv2 format not supported, update your files to PVRv3");
  }
...
static Image LoadASTC(const unsigned char* fileData, unsigned int fileSize) {
  ...
  if (fileDataPtr != NULL) {
    ...
    if ((astcHeader->id[3] != 0x5c) || (astcHeader->id[2] != 0xa1) || (astcHeader->id[1] != 0xab) ||
        (astcHeader->id[0] != 0x13)) {
      TRACELOG(LOG_WARNING, "IMAGE: ASTC file data not valid");
    } else {
      fileDataPtr += sizeof(ASTCHeader);  // Skip header

      // NOTE: Assuming Little Endian (could it be wrong?)
      image.width = 0x00000000 | ((int)astcHeader->width[2] << 16) | ((int)astcHeader->width[1] << 8) |
                    ((int)astcHeader->width[0]);
      image.height = 0x00000000 | ((int)astcHeader->height[2] << 16) | ((int)astcHeader->height[1] << 8) |
                     ((int)astcHeader->height[0]);
      ...
      if ((bpp == 8) || (bpp == 2)) {
        ...
        TRACELOG(LOG_WARNING, "IMAGE: ASTC block size configuration not supported");
    }
  }
