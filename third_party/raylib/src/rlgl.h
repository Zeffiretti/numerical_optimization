 ...
 *   #define RL_DEFAULT_BATCH_DRAWCALLS          256    // Default number of batch draw calls (by state changes: mode,
 *texture) #define RL_DEFAULT_BATCH_MAX_TEXTURE_UNITS    4    // Maximum number of textures units that can be activated
 ...
 *   #define RL_DEFAULT_SHADER_UNIFORM_NAME_NORMAL      "matNormal"         // normal matrix
 *(transpose(inverse(matModelView)) #define RL_DEFAULT_SHADER_UNIFORM_NAME_COLOR       "colDiffuse"        // color
 *diffuse (base tint color, multiplied by texture color) #define RL_DEFAULT_SHADER_SAMPLER2D_NAME_TEXTURE0  "texture0"
 *// texture0 (texture slot active 0) #define RL_DEFAULT_SHADER_SAMPLER2D_NAME_TEXTURE1  "texture1"          // texture1
 *(texture slot active 1) #define RL_DEFAULT_SHADER_SAMPLER2D_NAME_TEXTURE2  "texture2"          // texture2 (texture
 *slot active 2)
 *
 *   DEPENDENCIES:
 *
...
//----------------------------------------------------------------------------------
typedef enum { OPENGL_11 = 1, OPENGL_21, OPENGL_33, OPENGL_43, OPENGL_ES_20 } rlGlVersion;
...
RLAPI unsigned int rlLoadVertexBufferElement(void* buffer, int size,
                                             bool dynamic);  // Load a new attributes element buffer
RLAPI void rlUpdateVertexBuffer(unsigned int bufferId, void* data, int dataSize,
...
// Textures management
RLAPI unsigned int rlLoadTexture(void* data, int width, int height, int format,
...
RLAPI void rlGetGlTextureFormats(int format, int* glInternalFormat, int* glFormat,
                                 int* glType);                // Get OpenGL internal formats
...
RLAPI unsigned int rlCompileShader(const char* shaderCode,
                                   int type);  // Compile custom shader and return shader id (type: RL_VERTEX_SHADER,
...
RLAPI unsigned int rlLoadShaderBuffer(unsigned long long size, const void* data,
                                      int usageHint);  // Load shader storage buffer object (SSBO)
RLAPI void rlUnloadShaderBuffer(unsigned int ssboId);  // Unload shader storage buffer object (SSBO)
RLAPI void rlUpdateShaderBufferElements(unsigned int id, const void* data, unsigned long long dataSize,
                                        unsigned long long offset);  // Update SSBO buffer data
RLAPI unsigned long long rlGetShaderBufferSize(unsigned int id);     // Get SSBO buffer size
RLAPI void rlReadShaderBufferElements(unsigned int id, void* dest, unsigned long long count,
                                      unsigned long long offset);    // Bind SSBO buffer
RLAPI void rlBindShaderBuffer(unsigned int id, unsigned int index);  // Copy SSBO buffer data

// Buffer management
RLAPI void rlCopyBuffersElements(unsigned int destId, unsigned int srcId, unsigned long long destOffset,
                                 unsigned long long srcOffset, unsigned long long count);  // Copy SSBO buffer data
RLAPI void rlBindImageTexture(unsigned int id, unsigned int index, unsigned int format,
...
typedef struct rlglData {
  ...
  struct {
    ...
    unsigned int activeTextureId[RL_DEFAULT_BATCH_MAX_TEXTURE_UNITS];  // Active texture ids to be enabled on batch
                                                                       // drawing (0 active by default)
  ...
  struct {
    ...
    bool instancing;   // Instancing supported (GL_ANGLE_instanced_arrays, GL_EXT_draw_instanced +
    ...
    bool texCompDXT;   // DDS texture compression support (GL_EXT_texture_compression_s3tc,
                       // GL_WEBGL_compressed_texture_s3tc, GL_WEBKIT_WEBGL_compressed_texture_s3tc)
    bool texCompETC1;  // ETC1 texture compression support (GL_OES_compressed_ETC1_RGB8_texture,
                       // GL_WEBGL_compressed_texture_etc1)
    bool texCompETC2;  // ETC2/EAC texture compression support (GL_ARB_ES3_compatibility)
    bool texCompPVRT;  // PVR texture compression support (GL_IMG_texture_compression_pvrtc)
    bool texCompASTC;  // ASTC texture compression support (GL_KHR_texture_compression_astc_hdr,
...
#endif                                               // GRAPHICS_API_OPENGL_33 || GRAPHICS_API_OPENGL_ES2
#if defined(GRAPHICS_API_OPENGL_11)
static int rlGenTextureMipmapsData(unsigned char* data, int baseWidth,
...
void rlViewport(int x, int y, int width, int height) { glViewport(x, y, width, height); }
...
void rlEnd(void) {
  ...
  if (RLGL.State.vertexCounter >=
    ...
    // we need to call rlPopMatrix() before to recover *RLGL.State.currentMatrix (RLGL.State.modelview) for the next
    // forced draw call! If we have multiple matrix pushed, it will require "RLGL.State.stackCounter" pops before
    // launching the draw
...
static void GLAPIENTRY rlDebugMessageCallback(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length,
  ...
  // - #131185 - Buffer detailed info: Buffer object 1 (bound to GL_ELEMENT_ARRAY_BUFFER_ARB, usage hint is
  // GL_ENUM_88e4)
  //             will use VIDEO memory as the source for buffer object operations. (severity: low)
  // - #131218 - Program/shader state performance warning: Vertex shader in program 7 is being recompiled based on GL
...
void rlglInit(int width, int height) {
  ...
  if ((glDebugMessageCallback != NULL) && (glDebugMessageControl != NULL)) {
    ...
    // glDebugMessageControl(GL_DEBUG_SOURCE_API, GL_DEBUG_TYPE_ERROR, GL_DEBUG_SEVERITY_HIGH, 0, 0, GL_TRUE); // TODO:
    // Filter message

    // Debug context options:
    //  - GL_DEBUG_OUTPUT - Faster version but not useful for breakpoints
    //  - GL_DEBUG_OUTPUT_SYNCHRONUS - Callback is in sync with errors, so a breakpoint can be placed on the callback in
    //  order to get a stacktrace for the GL error
...
void rlDrawRenderBatch(rlRenderBatch* batch) {
  ...
  for (int eye = 0; eye < eyeCount; eye++) {
    ...
    if (RLGL.State.vertexCounter > 0) {
      ...
      // NOTE: Batch system accumulates calls by texture0 changes, additional textures are enabled for all the draw
      // calls
...
#  endif
...
#if defined(GRAPHICS_API_OPENGL_ES2)
  // NOTE: OpenGL ES 2.0 with no GL_OES_texture_npot support (i.e. WebGL) has limited NPOT support, so CLAMP_TO_EDGE
  // must be used
  if (RLGL.ExtSupported.texNPOT) {
  ...
  if (id > 0)
    TRACELOG(RL_LOG_INFO, "TEXTURE: [ID %i] Texture loaded successfully (%ix%i | %s | %i mipmaps)", id, width, height,
      ...
      if (data == NULL) {
        if (format < RL_PIXELFORMAT_COMPRESSED_DXT1_RGB) {
          if (format == RL_PIXELFORMAT_UNCOMPRESSED_R32G32B32) {
            // Instead of using a sized internal texture format (GL_RGB16F, GL_RGB32F), we let the driver to choose the
            // better format for us (GL_RGB)
          ...
          } else if ((format == RL_PIXELFORMAT_UNCOMPRESSED_R32) ||
                     (format == RL_PIXELFORMAT_UNCOMPRESSED_R32G32B32A32))
            TRACELOG(RL_LOG_WARNING, "TEXTURES: Cubemap requested format not supported");
          else
            glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, glInternalFormat, size, size, 0, glFormat, glType,
                         NULL);
        } else
        ...
        if (format < RL_PIXELFORMAT_COMPRESSED_DXT1_RGB)
          ...
          glCompressedTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, glInternalFormat, size, size, 0, dataSize,
                                 (unsigned char*)data + i * dataSize);
      }
...
void rlGetGlTextureFormats(int format, int* glInternalFormat, int* glFormat, int* glType) {
  ...
  *glType = -1;
  ...
  switch (format) {
    ...
    // NOTE: on OpenGL ES 2.0 (WebGL), internalFormat must match format and options allowed are: GL_LUMINANCE, GL_RGB,
    // GL_RGBA
  ...
  if (texIsPOT) {
    ...
    if (format == RL_PIXELFORMAT_UNCOMPRESSED_R8G8B8A8) {
      ...
      for (int level = 1; level < mipmapCount; level++) {
        glTexImage2D(GL_TEXTURE_2D, level, GL_RGBA8, mipWidth, mipHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                     (unsigned char*)texData + offset);
      ...
      *mipmaps = mipmapCount + 1;
      RL_FREE(
          texData);  // Once mipmaps have been generated and data has been uploaded to GPU VRAM, we can discard RAM data

      TRACELOG(RL_LOG_WARNING, "TEXTURE: [ID %i] Mipmaps generated manually on CPU side, total: %i", id, *mipmaps);
    } else
      TRACELOG(RL_LOG_WARNING, "TEXTURE: [ID %i] Failed to generate mipmaps for provided texture format", id);
  }
#endif
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
  if ((texIsPOT) || (RLGL.ExtSupported.texNPOT)) {
    // glHint(GL_GENERATE_MIPMAP_HINT, GL_DONT_CARE);   // Hint for mipmaps generation algorythm: GL_FASTEST, GL_NICEST,
    // GL_DONT_CARE
...
void* rlReadTexturePixels(unsigned int id, int width, int height, int format) {
  ...
  // NOTE: Each row written to or read from by OpenGL pixel operations like glGetTexImage are aligned to a 4 byte
  // boundary by default, which may add some padding. Use glPixelStorei to modify padding with the GL_[UN]PACK_ALIGNMENT
  // setting. GL_PACK_ALIGNMENT affects operations that read from OpenGL memory (glReadPixels, glGetTexImage, etc.)
...
// Draw vertex array
void rlDrawVertexArray(int offset, int count) { glDrawArrays(GL_TRIANGLES, offset, count); }
...
void rlDisableStatePointer(int vertexAttribType) { glDisableClientState(vertexAttribType); }
#endif
...
int rlGetLocationUniform(unsigned int shaderId, const char* uniformName) {
  ...
  if (location == -1)
    TRACELOG(RL_LOG_WARNING, "SHADER: [ID %i] Failed to find shader uniform: %s", shaderId, uniformName);
  else
    TRACELOG(RL_LOG_INFO, "SHADER: [ID %i] Shader uniform (%s) set at location: %i", shaderId, uniformName, location);
#endif
  return location;
}
...
int rlGetLocationAttrib(unsigned int shaderId, const char* attribName) {
  ...
  if (location == -1)
    TRACELOG(RL_LOG_WARNING, "SHADER: [ID %i] Failed to find shader attribute: %s", shaderId, attribName);
  else
    TRACELOG(RL_LOG_INFO, "SHADER: [ID %i] Shader attribute (%s) set at location: %i", shaderId, attribName, location);
#endif
  return location;
}
...
void rlSetUniformSampler(int locIndex, unsigned int textureId) {
  ...
  for (int i = 0; i < RL_DEFAULT_BATCH_MAX_TEXTURE_UNITS; i++)
    if (RLGL.State.activeTextureId[i] == textureId) return;
...
void rlUpdateShaderBufferElements(unsigned int id, const void* data, unsigned long long dataSize,
...
void rlCopyBuffersElements(unsigned int destId, unsigned int srcId, unsigned long long destOffset,
                           unsigned long long srcOffset, unsigned long long count) {
#if defined(GRAPHICS_API_OPENGL_43)
...
Matrix rlGetMatrixTransform(void) {
  ...
  // for (int i = RLGL.State.stackCounter; i > 0; i--) matStackTransform = rlMatrixMultiply(RLGL.State.stack[i],
  // matStackTransform);
...
static int rlGenTextureMipmapsData(unsigned char* data, int baseWidth, int baseHeight) {
  ...
  if (temp != NULL)
    ...
    TRACELOG(RL_LOG_WARNING, "TEXTURE: Failed to re-allocate required mipmaps memory");
