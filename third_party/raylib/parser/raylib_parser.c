    ...
    while (structLines[i][structLineOffset + j] != '}') {
      ...
      if (fieldStart != 0) {
        ...
        if (fieldLine[0] != '/')  // Field line is not a comment
        {
          ...
          GetDataTypeAndName(fieldLine, fieldEndPos, structs[i].fieldType[structs[i].fieldCount],
                             structs[i].fieldName[structs[i].fieldCount]);
  ...
  for (int i = 0; i < enumCount; i++) {
    ...
    for (int j = 1; j < MAX_ENUM_VALUES * 2; j++)  // Maximum number of lines following enum first line
    {
      ...
      if ((linePtr[0] >= 'A') && (linePtr[0] <= 'Z')) {
        ...
        int c = 0;
        while ((linePtr[c] != ',') && (linePtr[c] != ' ') && (linePtr[c] != '=') && (linePtr[c] != '\0')) {
  ...
  for (int i = 0; i < funcCount; i++) {
    ...
    for (int c = funcParamsStart; c < MAX_LINE_LENGTH; c++) {
      if (funcLines[i][c] == ',')  // Starts function parameters
      {
        ...
        GetDataTypeAndName(funcParamTypeName, funcParamTypeNameLen, funcs[i].paramType[funcs[i].paramCount],
                           funcs[i].paramName[funcs[i].paramCount]);
        ...
        if ((funcLines[i][c - 4] == 'v') && (funcLines[i][c - 3] == 'o') && (funcLines[i][c - 2] == 'i') &&
            (funcLines[i][c - 1] == 'd'))
          break;
...
static void ShowCommandLineInfo(void) {
  ...
  printf("USAGE:\n\n");
  printf(
      "    > raylib_parser [--help] [--input <filename.h>] [--output <filename.ext>] [--format <type>] [--define "
      "<DEF>]\n");
  ...
  printf(
      "    -d, --define <DEF>              : Define functions define (i.e. RLAPI for raylib.h, RMDEF for raymath.h, "
      "etc\n");
...
static char* CharReplace(char* text, char search, char replace) {
  for (int i = 0; text[i] != '\0'; i++)
    if (text[i] == search) text[i] = replace;
  return text;
}
    ...
    case DEFAULT: {
      ...
      for (int i = 0; i < structCount; i++) {
        ...
        for (int f = 0; f < structs[i].fieldCount; f++)
          fprintf(outFile, "  Field[%i]: %s %s %s\n", f + 1, structs[i].fieldType[f], structs[i].fieldName[f],
      ...
      */
      ...
      for (int i = 0; i < structCount; i++) {
        fprintf(outFile, "        <Struct name=\"%s\" fieldCount=\"%i\" desc=\"%s\">\n", structs[i].name,
                structs[i].fieldCount, structs[i].desc + 3);
        for (int f = 0; f < structs[i].fieldCount; f++) {
          fprintf(outFile, "            <Field type=\"%s\" name=\"%s\" desc=\"%s\" />\n", structs[i].fieldType[f],
                  structs[i].fieldName[f], structs[i].fieldDesc[f] + 3);
        }
      ...
      for (int i = 0; i < funcCount; i++) {
        fprintf(outFile, "        <Function name=\"%s\" retType=\"%s\" paramCount=\"%i\" desc=\"%s\">\n", funcs[i].name,
                funcs[i].retType, funcs[i].paramCount, funcs[i].desc + 3);
        for (int p = 0; p < funcs[i].paramCount; p++) {
          fprintf(outFile, "            <Param type=\"%s\" name=\"%s\" desc=\"%s\" />\n", funcs[i].paramType[p],
                  funcs[i].paramName[p], funcs[i].paramDesc[p] + 3);
        }
